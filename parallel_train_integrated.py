...
SHOW_WINDOW = (
    os.environ.get("SHOW_WINDOW", "false").lower() == "true"
)  # Set SHOW_WINDOW=true to show game windows
...
def read_instance_status(instance_id):
    ...
    try:
        status_file = Path("stats") / f"instance_{instance_id}_status.json"
...
def display_dashboard(processes):
    ...
    try:
        ...
        temp_str = f" | GPU Temp: {int(gpu_temp)}징횈C" if gpu_temp else ""
...
def start_parallel_training():
    ...
    try:
        for i in range(actual_instances):
            ...
            # Force SC2PATH to correct location (override any incorrect paths)

            env["SC2PATH"] = r"C:\Program Files (x86)\StarCraft II"

            # Use sys.executable to ensure the same Python environment

            if sys.platform == "win32":
                # Windows: Try 'py' first, fallback to sys.executable
                ...
                try:
                    process = subprocess.Popen(
                        ...
                        shell=False,
                        env=env,  # Pass environment with SHOW_WINDOW setting
                    )
