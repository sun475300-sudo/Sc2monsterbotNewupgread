...
class CombatTactics:
    """Encapsulates combat and defensive tactics, including micro helpers."""

    def __init__(self, bot: Any) -> None:
        self.bot = bot

    async def execute_combat(self) -> None:
        """Main combat execution, including baneling vs marines and unit micro."""
        b = self.bot
        try:
            ...
            if is_defenseless or is_outnumbered:
                ...
                if nearby_workers.exists:
                    ...
                    max_workers_to_pull = min(
                        max(int(worker_count * 0.3), 1), min(10, len(workers_list))
                    )
                    if worker_count < 5:
