...
class ChatManager:
    ...
    async def on_chat(self, chat_message):
        try:
            ...
            player_id = getattr(chat_message, "player_id", None)
            ...
            # [LOG] 수신 메시지 기록
            if not is_from_self and player_id is not None:
                self._log_msg(f"[CHAT RECV] Player {player_id}: {msg_text}")

            if not is_from_self and msg:
                response = ""
                # 1. GG / GLHF 반응
                if "gg" in msg and not self._gg_replied:
                    response = "GG! Good game."
                    self._gg_replied = True
                elif "gl hf" in msg or "glhf" in msg:
                    response = "gl hf! Have fun."
                
                # 2. 추가 감정 표현 반응
                elif "wp" in msg or "well played" in msg:
                    response = "Thanks! You too."
                elif "bg" in msg or "bad game" in msg:
                    response = "That was intense."
                elif "ty" in msg or "thanks" in msg:
                    response = "No problem."

                # 응답 전송
                if response:
                    await self.bot.chat_send(response)
                    self._log_msg(f"[CHAT RESPONSE] {response}")
