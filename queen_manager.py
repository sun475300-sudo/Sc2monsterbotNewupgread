...
class QueenManager:
    ...
    async def manage_queens(self):
        """
        Queen management - optimized with minimal error handling
        Note: Execution frequency is controlled by main scheduler (every 8 frames)

        Features:
        - Larva injection with queen-hatchery assignments for efficiency
        - Transfuse healing with priority system (queens > valuable units)
        - Smart buff checking to avoid duplicate injects
        """
        ...
        if intel and intel.cached_townhalls is not None:
            hatcheries = intel.cached_townhalls.ready
        else:
        ...
        if not queens.exists:
            ...
            if self.bot.can_afford(UnitTypeId.QUEEN):
                ...
                idle_hatcheries = [h for h in hatcheries if h.is_idle]
                if idle_hatcheries:
                    try:
        ...
        hatchery_list = list(hatcheries) if hatcheries.exists else []
                        ...
                        if queen_to_use.is_idle or queen_to_use.is_moving:
                            try:
                                ...
                                if getattr(self.bot, "iteration", 0) % 50 == 0:
                                    print(
                                        f"[SUCCESS] {hatch.tag}에 펌핑 완료! 여왕 에너지: {queen_to_use.energy}"
                                    )
                            except Exception:
