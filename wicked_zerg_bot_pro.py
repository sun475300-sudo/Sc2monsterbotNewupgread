...
"""
================================================================================
        🎮 Wicked Zerg AI - Professional Main Bot (wicked_zerg_bot_pro.py)
================================================================================
Final version with all advanced features integrated

Integrated Systems:
    1. IntelManager - Global Intelligence (Blackboard)
    2. MicroController - Potential Fields Unit Spreading
    3. HeatmapScout - Grid-based Scouting
    4. Existing Managers (Economy, Production, Combat, Scout)
================================================================================
"""
...
if sys.platform == "win32":
    try:
        if hasattr(sys.stdout, "detach"):
            sys.stdout = io.TextIOWrapper(sys.stdout.detach(), encoding="utf-8")  # type: ignore[attr-defined]
        if hasattr(sys.stderr, "detach"):
            sys.stderr = io.TextIOWrapper(sys.stderr.detach(), encoding="utf-8")  # type: ignore[attr-defined]
    except:
        pass
...
# strategy_decision_engine has been retired (moved to backups/retired)
# Functionality integrated into StrategyAnalyzer
...
try:
    ...
    logger.warning("bot_api_connector not found - dashboard integration disabled")

# Optional imports (may not exist if files were moved to backup)
# Suppress import warnings for optional modules
# CRITICAL: early_defense_manager.py may not exist - commented out to prevent import errors
# from early_defense_manager import EarlyDefenseManager  # type: ignore[import-untyped]
EarlyDefenseManager = None  # Set to None - will use DummyDefenseManager

try:
    from hot_loader import HotLoader  # type: ignore[import-untyped]
except (ImportError, ModuleNotFoundError):
    HotLoader = None  # type: ignore[assignment, misc]

try:
    from gas_maximizer import GasMaximizer  # type: ignore[import-untyped]
except (ImportError, ModuleNotFoundError):
    GasMaximizer = None  # type: ignore[assignment, misc]
...
try:
    ...
    class DebugVisualizer:
        ...
        def generate_debug_chart(self, *args, **kwargs):
            pass
...
class WickedZergBotPro(BotAI):
    ...
    def __init__(
        ...
        self.log_path = os.path.join(script_dir, "logs")
        ...
        if self.use_neural_network:
            try:
                ...
                if ZergNet:
                    model = ZergNet(input_size=5, hidden_size=64, output_size=4)  # type: ignore[misc]
                else:
                    model = None
                ...
                if ReinforcementLearner and model:
                    try:
                        self.neural_network = ReinforcementLearner(model, learning_rate=0.001)  # type: ignore[misc]
                        print(f"[OK] Neural network initialized")
                        
                        # ⚠️ CRITICAL: 저장된 모델 가중치 로드 (AI Arena 환경)
                        if not train_mode:  # 경쟁 모드에서만 로드
                            from pathlib import Path
                            ...
                            if model_path.exists() and torch is not None:
                                try:
                                    ...
                                    print(f"[INFO] Starting with fresh model (untrained)")
                            else:
    ...
    async def on_building_construction_complete(self, unit):
        """
        Immediately assign workers when Hatchery or Gas building completes

        This method is automatically called when a building completes,
        and immediately assigns workers to new expansions or gas buildings.
        """
        try:
            ...
            # Silent fail - building completion handler shouldn't crash the bot
            if self.iteration % 200 == 0:
                print(f"[WARNING] on_building_construction_complete error: {e}")

    async def on_start(self):
        """Game start - Enhanced error handling for Micro Ladder compatibility"""
        try:
            ...
            if self.opponent_race:
                print(f"⚔️ Opponent Race: {self.opponent_race.name}")
            print("=" * 70)
            ...
            if not self.opponent_race:
                ...
                self.opponent_race = "Terran"  # Default value
            ...
            # Initialize tech building ready flags (필수: 게임 시작 시 초기화)
            # 이 초기화가 없으면 spawning_pool_ready_flag가 정의되지 않음
            ...
            try:
                # CounterPunchManager merged into StrategyAnalyzer
                pass  # No longer needed - merged into StrategyAnalyzer
            except Exception as e:
                print(f"[WARNING] Failed to initialize CounterPunchManager: {e}")

                class DummyCounter:
                    async def apply_counter_strategy(self):
                        pass

                self.counter_punch = DummyCounter()

            # DummyGas class definition (used if GasMaximizer is not available)
            ...
            try:
                if GasMaximizer is not None:
                    ...
                    self.gas_maximizer = DummyGasManager()  # type: ignore[assignment]
            except Exception as e:
                print(f"[WARNING] Failed to initialize GasMaximizer: {e}")
                self.gas_maximizer = DummyGasManager()  # type: ignore[assignment]

            try:
                # OpponentTracker merged into StrategyAnalyzer
                pass
            except Exception as e:
                print(f"[WARNING] Failed to initialize OpponentTracker: {e}")

                class DummyTracker:
                    def set_opponent(self, name, *args, **kwargs):
                        pass

                    def record_match_result(self, result, *args, **kwargs):
                        pass

                    def should_use_aggressive_build(self, name=None, *args, **kwargs):
                        return False

                    def get_race_win_rate(self, *args, **kwargs):
                        return 0.0

                    def get_opponent_stats(self, *args, **kwargs):
                        return {}

                # opponent_tracker is now part of strategy_analyzer
                pass
            ...
            class DummyDefenseManager:
                """Common dummy defense manager used when EarlyDefenseManager is unavailable"""
            ...
            # Send chat messages (may fail in some ladder modes)
            try:
                await self.chat_send(f"🎮 위키드 저그 AI ({self.personality.upper()}) 연동 성공!")
            ...
            if self.opponent_race:
                self.write_log(f"Opponent race: {self.opponent_race.name}", "INFO")
    ...
    async def on_step(self, iteration: int):
        """
        메인 게임 루프 (통합형: Micro Ladder + Standard Melee 지원)

        구조:
        1. 공용 전투 로직 (적 유닛이 있으면 무조건 공격) - Micro와 Melee 모두
        2. 메일리 전용 로직 (본진이 있을 때만 실행)
        3. 마이크로 맵일 때는 적을 찾아 맵 전체를 수색
        """
        try:
            # 0️⃣ 게임 종료 플래그 체크 (GG 후 즉시 종료 보장)
            if hasattr(self, "game_ended") and self.game_ended:
                return  # Game ended, exit immediately - don't process any more steps

            # 0️⃣ iteration 업데이트 (가장 먼저) - 중복 선언 방지
            ...
            # [에러 해결] opponent_race가 None이면 enemy_race로 설정
            ...
            # 22프레임(약 1초)마다 터미널에 실시간 상태 출력 (인코딩 에러 방지 - 영어 사용)
            if iteration % 22 == 0:
                try:
                    ...
                    # Build dashboard string (English format to avoid encoding issues)
                    # Format: [GAME #XXX] TIME: MM:SS | MIN: XXXX | SUPPLY: XX/XX | UNITS: XXX | LAST: XXX
                    ...
                    # Use sys.stdout.write and flush for real-time single-line updates
                    sys.stdout.write(dashboard)
                    sys.stdout.flush()

                    # Update status file for parallel training dashboard (if instance_id is set)
                    try:
                        instance_id = getattr(self, "instance_id", 0)
                        if instance_id > 0:  # Only update if in parallel mode
                            import json
                            from pathlib import Path
                        ...
                        pass  # Silent fail - status file update is optional
                    ...
                    pass  # Silent fail to avoid interrupting game flow
            ...
            # ⚠️ CRITICAL: Supply Block Prevention (Run Every Frame)
            ...
            if self.supply_left < 3 and not self.already_pending(UnitTypeId.OVERLORD):
                # type: ignore[operator] keeps Pylance quiet on burnysc2 can_afford wrapper
                if self.can_afford(UnitTypeId.OVERLORD) and self.larva.exists:  # type: ignore[operator]
                    try:
            ...
            # 🛡️ 통신 안정성 확보: 8프레임마다 한 번씩만 로직 실행 (CPU 부하 대폭 감소)
            # step_interval을 직접 줄 수 없으므로 봇 내부에서 처리
            # PERFORMANCE: Increased from 4 to 8 frames for 50% additional CPU load reduction
            if iteration % 8 != 0:
                return
            ...
            # 0.5️⃣ 항복 체크 (Surrender Logic) - 학습 효율 향상을 위한 조기 항복
            # 88프레임마다 체크 (약 4초마다, CPU 부하 대폭 감소)
            ...
            # 1️⃣ 공용 전투 로직 (Micro Ladder + Standard Melee 공통)
            # 적 유닛이 있으면 무조건 공격 (모든 모드에서 실행)
            ...
            # 1.5️⃣ 긴급 일꾼 동원 방어 (Worker Defense) - 15프레임마다 실행
            # 병력이 부족할 때 일꾼을 동원하여 기지 방어
            ...
            # 1.5️⃣ 20초 초고속 정찰 (대군주 활용) - 40프레임마다 실행
            ...
            # 일꾼이 적 기지로 달려가 허무하게 잡히는 상황 방지
            # 안전 구역을 벗어나면 즉시 가장 가까운 아군 기지로 복귀
            ...
            # 1.7️⃣ 실시간 게임 진행도 채팅 알림 (5초마다, 약 112 프레임)
            # 빌드 단계, 인구수, 자원, 상대 테크 정보를 채팅창에 출력
            if self.iteration % 112 == 0 and self.townhalls.exists:
                await self._send_game_progress_to_chat()

            # 1.7-1️⃣ 실시간 내면적 판단 근거 노출 (20초마다, 약 448 프레임)
            # 봇이 자신의 의사결정 근거를 실시간으로 설명
            ...
            # 1.8️⃣ 실시간 승리 확률 대조 및 채팅 출력 (20초마다, 약 448 프레임)
            # 상대 테크와 내 유닛 조합을 비교하여 승률을 계산하고 채팅으로 출력
            # PERFORMANCE: Increased from 224 to 448 frames for CPU load reduction
            if self.iteration % 448 == 0 and self.townhalls.exists:
            ...
            # 2️⃣ 메일리 전용 로직 (본진이 있을 때만 실행)
            # Standard Melee Ladder: 자원 채취, 건물 건설, 유닛 생산
            if self.townhalls.exists:
                ...
                # 3️⃣ 마이크로 맵 전용 로직 (본진이 없을 때)
                # Micro Ladder: 적을 찾아 맵 전체를 수색하는 로직만 수행
    ...
    async def _execute_melee_ladder_logic(self, iteration: int):
        ...
        try:
            ...
            # 서버 환경에 따라 on_start 시점에 유닛 정보가 늦게 로드될 수 있음
            if not self.townhalls.exists:
                # Townhalls가 없으면 매크로 로직 실행 불가 - 조용히 리턴
                return
            ...
            # 🛡️ 초반 수비 진영 구성 (Torches 맵 등 대응)
            # main_base_ramp를 활용하여 입구에 저글링 배치
            # PERFORMANCE: Increased from 60 to 120 frames for additional CPU load reduction
            if (
                iteration % 120 == 0 and self.time < 180
            ):  # 초반 3분 동안만, 120프레임마다 체크 (CPU 부하 대폭 감소)
                try:
                    # main_base_ramp가 있는지 확인
                    ...
                    # Ramp 정보가 없거나 접근 불가능한 경우 조용히 무시
                    pass
            ...
            # 0. 방패 수비 로직 가동 (0~3분대 날빌 방어 - 최우선)
            # 이 함수가 실행되는 동안에는 일벌레 생산보다 수비가 우선됨
            ...
            if iteration % 8 == 0:
                ...
                # 매니저 자동 초기화 (None 체크) - 초기화 후에는 체크 생략
            ...
            # CPU/GPU 리소스 균형 최적화: 1:1 비율 유지
            # CPU와 GPU 작업을 동일한 빈도로 실행하여 리소스 사용 균형 유지
            # 모든 CPU 집약적 작업과 GPU 작업을 동기화하여 효율적인 리소스 활용
            ...
            # 1. 지능 시스템 데이터 갱신 (모든 매니저가 공유)
            # Intel Manager: Every frame - Data consistency (all managers share this)
            if self.intel is not None:
                try:
                    self.intel.update(iteration)
                except Exception as e:
                    if iteration - self.last_error_log_frame >= 50:
                        print(f"[WARNING] IntelManager.update() error: {e}")
                        self.last_error_log_frame = iteration

            # 2. 마이크로 컨트롤 (전투 반응성을 위해 매 프레임 유지)
            # Micro Controller: Every frame - Combat responsiveness
            # 🚀 수정: MicroController에는 execute() 메서드가 없음
            # 마이크로 컨트롤은 CombatManager의 _micro_units()에서 처리됨
            # 여기서는 추가 마이크로 로직이 필요하면 처리
            # (현재는 CombatManager에서 처리하므로 여기서는 생략)
            ...
            if iteration % 22 == 0:
                # Production Manager
                # 생산 관리 (패닉 모드일 때는 저글링/여왕/가시촉수만 생산)
                if self.production is not None:
                    try:
                        if self.early_defense is not None and self.early_defense.is_panic_mode():
                            # 패닉 모드: 저글링과 여왕만 생산, 가시촉수 건설
                            await self._panic_mode_production()
                        else:
                            # 일반 모드: 정상 생산
                            # Note: ideal_composition is determined in _build_army_aggressive() to avoid duplicate calls
                            # Cache it if already determined to avoid redundant calculation
                            if (
                                not hasattr(self, "current_build_plan")
                                or "ideal_composition" not in self.current_build_plan
                            ):
                                ideal_composition = await self._determine_ideal_composition()
                                if not hasattr(self, "current_build_plan"):
                                    self.current_build_plan = {}
                                self.current_build_plan["ideal_composition"] = ideal_composition
                        ...
                        if iteration - self.last_error_log_frame >= 50:
                            if iteration % 200 == 0:
                                print(f"[WARNING] ProductionManager.update() error: {e}")
                            self.last_error_log_frame = iteration

                # Queen Manager: Larva injection (synchronized with production)
                # 🚀 수정: QueenManager에는 inject_larva()가 없고 manage_queens()가 있음
            ...
            # [4] Resource Management: Every 22 frames (synchronized with economy)
            if iteration % 22 == 0:
                # CRITICAL: Force resource dump when minerals exceed 1000
                # This prevents resource accumulation beyond 1000 minerals
                if self.minerals > 1000:
                    try:
                        await self._force_resource_dump()
                    except Exception as e:
                        if iteration - self.last_error_log_frame >= 50:
                            if iteration % 200 == 0:  # Throttle print statements (every ~9 seconds)
                                print(f"[WARNING] _force_resource_dump() 오류: {e}")
                            self.last_error_log_frame = iteration
            ...
            # 통계 분석은 가끔 해도 결과에 지장 없음 (~4-5 seconds)
            if iteration % 100 == 0:
                ...
                # Production diagnosis - Check why units are not being produced
                try:
                ...
                # Clear cached values to refresh (자율적 캐시 관리)
                ...
                try:
                    ...
                    # GPU 활용: 신경망 기반 행동 선택 (CPU와 동일한 빈도로 실행하여 1:1 비율 유지)
            ...
            # 사용 안 하는 변수들을 메모리에서 즉시 삭제하여 프리징 방지
            # PERFORMANCE: Increased from 250 to 500 frames for CPU load reduction
            if iteration % 500 == 0:
                gc.collect()
                # GPU 메모리도 정리 (CUDA 사용 시) - GPU 사용률 30% 제한을 위한 정리
            ...
            # 16. GPU 메모리 정리 (매 1000 프레임마다, 약 45초)
            ...
            if iteration - self.last_error_log_frame >= 50:
                ...
                #         logger.error(f"{instance_tag} 📍 위치: {error_info['file']}:{error_info['line']}")
                #         if error_info.get("code_line"):
                #             logger.error(f"{instance_tag} 💻 코드: {error_info['code_line']}")
                #
                #     if error_info.get("suggested_fixes"):
                #         logger.error(f"{instance_tag} 🔧 수정 제안: {len(error_info['suggested_fixes'])}개 발견")
                ...
                #     logger.error(f"⚠️ 로직 에러 발생 (프레임 {iteration}): {e}")
                #     traceback.print_exc()
    ...
    def _decide_strategy(self):
        ...
        if self.strategy_hub:
            # StrategyHub.update_strategy()가 모든 전략 로직 처리
    ...
    async def _execute_combat(self):
        ...
        # 마이크로 컨트롤러 초기화 (핫 리로드 지원)
        # 🚀 수정: micro_manager -> micro_controller (파일명 일치)
        if not hasattr(self, "unit_micro"):
            try:
                # MicroController는 이미 상단에서 import됨
                # self.micro가 이미 초기화되어 있으면 재사용
                if self.micro is not None:
                    self.unit_micro = self.micro  # 기존 MicroController 인스턴스 재사용
                else:
        ...
        # 🚀 수정: intel이 None일 수 있으므로 안전하게 체크
        ...
        if (
            ...
            # 궤멸충 부식성 담즙 저격 (Serral 스타일) - use cached units
            if hasattr(cached_units, "__call__"):
            ...
            # 방어 시에도 마이크로 매니저 사용 (use cached units)
            army_types = {
                UnitTypeId.ZERGLING,
                UnitTypeId.ROACH,
                UnitTypeId.HYDRALISK,
            ...
            # 저저전(ZvZ) 마이크로: 방어 모드에서도 체력 낮은 저글링 뒤로 빼기
    ...
    async def fix_production_bottleneck(self):
        ...
        try:
            # 0순위: 필수 테크 건물 건설 확인 (최우선)
            # 산란못이 없으면 저글링을 생산할 수 없으므로 최우선으로 건설
            # Build at 17-18 supply for optimal build order
            if (
                not self.units(UnitTypeId.SPAWNINGPOOL).exists
                and self.already_pending(UnitTypeId.SPAWNINGPOOL) == 0
            ):
                # Build Spawning Pool at 17-18 supply (optimal timing)
                supply_used = getattr(self, "supply_used", 0)
                should_build_pool = supply_used >= 17 and supply_used <= 20

                # Also build if we have enough resources and it's critical (emergency fallback)
                emergency_build = supply_used > 20 and self.can_afford(UnitTypeId.SPAWNINGPOOL)
                        ...
                        if self.townhalls.exists:
                            try:
                                ...
                                await self.build(
                                    ...
                                    near=main_base.position.towards(self.game_info.map_center, 5),
                                )
                                if self.iteration % 50 == 0:
                                    print(
                                        f"[TECH BUILD] [{int(self.time)}s] Building Spawning Pool at {supply_used} supply (required for Zergling production)"
                                    )
                                return  # 건설 시작했으면 이번 프레임은 종료
                            except Exception as e:
                                if self.iteration % 100 == 0:
                                    print(
                                        f"[TECH BUILD] [{int(self.time)}s] Failed to build Spawning Pool: {e}"
                                    )

            larvae = self.units(UnitTypeId.LARVA)

            # 라바가 없는데 돈이 남으면 해처리 추가 건설 (미래를 위한 투자)
            ...
            if self.supply_left < 4 and self.supply_cap < 200:
                # type: ignore[operator] keeps can_afford wrapper from tripping Pylance
                if self.can_afford(UnitTypeId.OVERLORD) and larvae.exists:  # type: ignore[operator]
                    try:
            ...
            # 2순위: 병력 (돈 아끼지 말고 쏟아붓기)
            # 산란못이 있는지 확인 (완성된 것 또는 99%+ 진행 중인 것)
            ...
            if spawning_pools_ready:
                ...
                larvae_list = list(larvae)
                produced_count = 0
                max_production = min(10, len(larvae_list))  # 한 번에 최대 10마리씩 생산
            ...
            # 3순위: 바퀴 (로치 워렌이 있으면)
            # 로치 워렌이 있는지 확인 (완성된 것 또는 99%+ 진행 중인 것)
            ...
            if roach_warrens_ready:
                ...
                larvae_list = list(larvae)
                produced_count = 0
                max_production = min(5, len(larvae_list))  # 최대 5마리씩
                for i, larva in enumerate(larvae_list[:max_production]):
                    if not larva.is_ready:
                        continue
                    # type: ignore[operator] keeps can_afford wrapper from type errors
            ...
            # 4순위: 히드라 (히드라 둥지가 있으면)
            # 히드라 둥지가 있는지 확인 (완성된 것 또는 99%+ 진행 중인 것)
    ...
    async def _diagnose_production_status(self, iteration: int):
        ...
        try:
            if iteration % 50 == 0:  # Every ~2 seconds
                larvae = self.units(UnitTypeId.LARVA)
                ...
                try:
                    if pool_query.ready.exists:
                        ...
                        try:
                            ...
                            if iteration % 50 == 0:
                                print(
                                    f"[DEBUG] Spawning Pool detected: Building... ({pool_progress*100:.1f}%)"
                                )
                ...
                if pool_ready_now:
                    if not self.spawning_pool_ready_flag:  # Flag 변경 시에만 로그
                        print("\n" + "="*80)
                    ...
                    for hatch in self.townhalls.ready:
                        ...
                        if close_spines.amount < 2:
                            try:
                                ...
                                continue  # 건설 실패 시 다음 해처리 시도

                # 2. 추가 부화장(Macro Hatchery) 건설 (라바 확보용)
                if (
    ...
    async def _defensive_rally(self):
        ...
        try:
            ...
            if enemy_near_base and enemy_near_base.exists:
                ...
                # 평상시: 앞마당 입구(Natural Expansion) 근처에 병력 집결하여 방어선 형성
    ...
    async def _worker_defense_emergency(self):
        ...
        try:
            # 1. 아군 기지 주변(반경 15)에 적 유닛이 있는지 확인
            enemy_units_obj = getattr(self, "known_enemy_units", None) or getattr(
                self, "enemy_units", None
            )  # type: ignore[attr-defined]
            ...
            if not near_enemies.exists:
                # 적이 없으면 일꾼들을 즉시 자원으로 복귀 (GG 선언 전까지 효율 극대화)
            ...
            # 🛡️ NEW: Proactive Worker Retreat - 일꾼 보호 강화
            # 적이 접근하면 일꾼을 먼저 대피시켜 생존률 향상
                        ...
                        try:
                            ...
                            if nearest_townhall and worker.distance_to(nearest_townhall) < 10:
                                ...
                                if safe_minerals.exists:
                                    worker.gather(safe_minerals.closest_to(worker.position))
                                else:
            ...
            # 비상 상황: 적과 가장 가까운 일꾼들 위주로 동원 (자원 수급 타격 최소화)
            if is_defenseless or is_outnumbered:
                # 거리 제한: 침입 경로에 있는 일꾼들만 전투에 참여 (distance_to < 12)
                nearby_workers = self.workers.filter(
                    lambda w: any(w.distance_to(e) < 12 for e in near_enemies)
                )

                if nearby_workers.exists:
                    # 체력이 높은 일꾼부터 정렬 (체력 100% 우선)
                    workers_list = sorted(
                    ...
                    # 최대 30% 또는 최대 10기 동원 (체력 높은 순서대로)
                    max_workers_to_pull = min(
                        max(int(worker_count * 0.3), 1),  # 최소 1기
                        min(10, len(workers_list)),  # 최대 10기
                    )

                    # 만약 일꾼이 5기 미만이면 모두 동원 (절체절명)
                    if worker_count < 5:
                        max_workers_to_pull = len(workers_list)

                    defense_workers = workers_list[:max_workers_to_pull]

                    for drone in defense_workers:
                        try:
                            # 이미 공격 중이 아니라면 공격 명령 하달
                            if not drone.is_attacking:
                                target = near_enemies.closest_to(drone.position)
                                if target:
                                    drone.attack(target)
                        except Exception:
                            pass

                # 4. GG 선언과 연동: 일꾼을 다 동원했는데도 본진 건물이 파괴되면 GG
    ...
    async def _fast_scouting_20_seconds(self):
        ...
        try:
            ...
            # ScoutManager가 있으면 ScoutManager에 위임
            if hasattr(self, "scout") and self.scout:
                # ScoutManager의 fast_scouting_20_seconds가 이미 호출되었는지 확인
            ...
            # 1. 대군주 우선 정찰 (안전하고 빠름)
            # CRITICAL: Overlords (대군주) are for scouting and vision ONLY, NOT combat
                ...
                try:
                    if overlord_count == 1:
                        ...
                        if self.enemy_start_locations:
                            ...
                            if self.iteration % 50 == 0:
                                print(
                                    f"[SCOUT] [{int(self.time)}s] 대군주 #1 정찰 중: 적 확장 기지 입구"
                                )
                        ...
                        if self.townhalls.exists:
                            ...
                            if self.iteration % 50 == 0:
                                print(
                                    f"[SCOUT] [{int(self.time)}s] 대군주 #2 배치 중: 아군 확장 기지 시야 확보"
                                )
                    else:
                        ...
                        if self.iteration % 50 == 0:
                            print(
                                f"[SCOUT] [{int(self.time)}s] 대군주 #{overlord_count} 배치 중: 고지 시야 확보"
                            )

                    return  # 대군주 정찰을 보냈으면 종료
                except Exception:
                    pass

            # 2. 대군주가 없으면 저글링 정찰 (대군주가 생산되면 대체)
    ...
    async def _enforce_worker_safe_zone(self):
        ...
        try:
            ...
            # 2. 적 기지 위치 파악 (적 기지 진입 차단용)
            enemy_base = None
            if self.enemy_start_locations and len(self.enemy_start_locations) > 0:
                enemy_base = self.enemy_start_locations[0]

            # 3. 안전 거리 설정 (매우 엄격하게: 25.0 - 미네랄 필드와 가스 필드 범위 내)
            # 해처리 주변 미네랄 필드는 보통 10-15 거리 내에 있으므로, 25.0이면 충분히 안전
            safe_distance = 25.0
            enemy_base_safe_distance = 60.0  # 적 기지 진입 차단 거리

            # 4. 모든 드론을 검사
            for drone in self.workers:
                try:
                    # 드론과 가장 가까운 아군 기지와의 거리 계산
                    ...
                    if hasattr(drone, "orders") and drone.orders:
                        for order in drone.orders:
                            # 건물 건설 명령인지 확인 (AbilityId.BUILD_* 패턴)
                            if hasattr(order, "ability") and order.ability:
                    ...
                    if closest_mineral:
                        ...
                        # 미네랄 필드가 기지 근처(15.0 이내)에 있고, 일꾼이 그 미네랄 필드 근처(5.0 이내)에 있으면 허용
                    ...
                    if not is_near_resource and closest_gas:
                        ...
                        # 가스 필드가 기지 근처(15.0 이내)에 있고, 일꾼이 그 가스 필드 근처(5.0 이내)에 있으면 허용
                    ...
                    if not is_gathering:
                        ...
                        if minerals_near_base.exists:
                            ...
                            # 미네랄 필드가 없으면 기지로 직접 이동 (다음 프레임에 채취)
                            drone.move(closest_base.position)
                    ...
                    # 개별 드론 처리 오류는 조용히 무시 (다른 드론 처리 계속)
                    pass

        except Exception as e:
            # 전체 로직 오류는 조용히 무시 (봇 크래시 방지)
            current_iteration = getattr(self, "iteration", 0)
            if current_iteration - getattr(self, "last_error_log_frame", 0) >= 100:
    ...
    async def _display_debug_to_chat(self, iteration: int):
        """
        게임 채팅창에 실시간 디버그 정보 표시

        채팅창에 봇의 현재 상태를 표시합니다.
        - 자원 (미네랄/가스)
        - 인구수 (사용/최대/남음)
        - 드론 수
        - 병력 수
        ...
        try:
            ...
            # 2. 순수 병력 수 계산 (화이트리스트 방식 - 저글링 이상 전투 유닛만)
            army_units = self.units.filter(lambda u: u.type_id in self.combat_unit_types)
            army_count = (
            ...
            # 7. 채팅으로 전송 (1초마다는 너무 자주이므로 5초마다)
            if iteration % 112 == 0:  # 약 5초마다
                await self.chat_send(debug_msg)

        except Exception:
            # 전체 함수 실패 시 조용히 무시 (게임 중단 방지)
            pass

    # 📊 훈련 모니터링: 관전 가이드 정보 출력
    async def _display_training_monitoring(self, iteration: int):
        """
        Display training monitoring information for spectator guidance

        Checks:
        1. Supply explosion (Swarm Check) - Population growth rate
        2. Mineral control (Economy Check) - Resource spending efficiency
        3. Unit composition harmony (Composition Check) - Unit ratio balance
        4. Macro hatchery (Larva Supply) - Hatchery construction for larva production
        """
        try:
            # Only display every 10 seconds (224 frames) to avoid spam
            if iteration % 224 != 0:
                return

            game_time = int(self.time)

            # 1. 인구수 폭발 체크 (Swarm Check)
            # 중반(5~8분)에 인구수가 100을 넘어 150~200을 향해 올라가는지 확인
            if 300 <= game_time <= 480:  # 5~8분
                supply_army = self.supply_army
                supply_cap = self.supply_cap
                supply_growth_rate = supply_army / max(game_time / 60, 1)  # Supply per minute

                if supply_army < 100:
                    await self.chat_send(
                        f"⚠️ [Swarm Check] 인구수 {supply_army}/{supply_cap} - 목표: 100+ (현재: {supply_growth_rate:.1f}/분)"
                    )
                elif supply_army < 150:
                    await self.chat_send(
                        f"✅ [Swarm Check] 인구수 {supply_army}/{supply_cap} - 양호! 목표: 150+"
                    )
                else:
                    await self.chat_send(
                        f"🔥 [Swarm Check] 인구수 {supply_army}/{supply_cap} - 폭발적 성장!"
                    )

            # 2. 미네랄 통제 체크 (Economy Check)
            # 미네랄이 1,000 근처로 가려고 할 때 자원 소모 로직 작동 확인
            if self.minerals > 800:
                # Check pending units (units being produced)
                pending_zerglings = self.already_pending(UnitTypeId.ZERGLING)
                pending_roaches = self.already_pending(UnitTypeId.ROACH)
                pending_hydralisks = self.already_pending(UnitTypeId.HYDRALISK)
                total_pending = pending_zerglings + pending_roaches + pending_hydralisks

                if total_pending > 0:
                    await self.chat_send(
                        f"💰 [Economy Check] 미네랄 {int(self.minerals)}M - {total_pending}개 유닛 생산 중 (자원 소모 정상)"
                    )
                else:
                    larva_count = self.units(UnitTypeId.LARVA).amount
                    if larva_count == 0:
                        await self.chat_send(
                            f"⚠️ [Economy Check] 미네랄 {int(self.minerals)}M - 라바 {larva_count}개! 여왕 인젝트 필요"
                        )
                    else:
                        await self.chat_send(
                            f"⚠️ [Economy Check] 미네랄 {int(self.minerals)}M - 생산 없음 (라바: {larva_count}개)"
                        )

            # 3. 병력 조합 조화 체크 (Composition Check)
            # 가스 상황에 따른 유닛 비율 확인
            ...
            if total_army > 0:
                ...
                if self.vespene >= 100:
                    ...
                    if hydra_ratio + roach_ratio < 0.5:
                        await self.chat_send(
                            f"⚠️ [Composition] 가스 충분({int(self.vespene)}G) - 히드라/바퀴 비율 낮음 (L{ling_ratio:.0%} R{roach_ratio:.0%} H{hydra_ratio:.0%})"
                        )
                    else:
                        await self.chat_send(
                    ...
                    if ling_ratio < 0.6:
                        await self.chat_send(
                            f"⚠️ [Composition] 가스 부족({int(self.vespene)}G) - 저글링 비율 낮음 (L{ling_ratio:.0%} R{roach_ratio:.0%} H{hydra_ratio:.0%})"
                        )

            # 4. 매크로 해처리 체크 (Larva Supply)
            # 해처리 개수와 라바 수 확인
            hatchery_count = self.townhalls.amount
            larva_count = self.units(UnitTypeId.LARVA).amount

            if game_time >= 300:  # 5분 이후
                if hatchery_count < 3:
                    await self.chat_send(
                        f"⚠️ [Larva Supply] 해처리 {hatchery_count}개 - 매크로 해처리 필요!"
                    )
                elif larva_count < 3:
                    await self.chat_send(
                        f"⚠️ [Larva Supply] 라바 {larva_count}개 부족 - 여왕 인젝트 확인 필요"
                    )
                else:
                    await self.chat_send(
    ...
    async def _express_bot_thoughts(self, iteration: int):
        ...
        try:
            ...
            # 미네랄이 1000 이상이면 공격적 모드
            if self.minerals > 1000:
                mood = "🔥 공격적"
                thought = f"병력을 모아 한 번에 끝내겠습니다. (보유 미네랄: {self.minerals})"
            # 미네랄이 500-1000 사이면 평온함
            elif self.minerals > 500:
                mood = "✨ 평온함"
                thought = f"안정적으로 자원을 확보하며 다음 단계를 구상 중입니다. (보유 미네랄: {self.minerals})"
            # 미네랄이 500 미만이면 신중함
            else:
                mood = "🛡️ 신중함"
                thought = f"자원이 부족합니다. 안전하게 멀티를 늘리는 중입니다. (보유 미네랄: {self.minerals})"

            # 2. 병력 상황 분석 (화이트리스트 방식 - 저글링 이상 전투 유닛만)
            army_units = self.units.filter(lambda u: u.type_id in self.combat_unit_types)
            army_count = (
                army_units.amount if hasattr(army_units, "amount") else len(list(army_units))
            )

            if army_count == 0 and self.minerals > 1000:
                thought += " ⚠️ 병력이 없어 자원을 병력으로 전환하겠습니다."

            # 3. 건설 상황 분석
            spawning_pools = list(self.units(UnitTypeId.SPAWNINGPOOL).structure)
            roach_warrens = list(self.units(UnitTypeId.ROACHWARREN).structure)

            if not spawning_pools and self.minerals >= 200:
                thought += " 🏗️ 산란못이 없어 건설이 필요합니다."
            elif not roach_warrens and self.minerals >= 150 and self.vespene >= 50:
                thought += " ⚔️ 바퀴 소굴을 준비 중입니다."

            # 4. 채팅으로 전송
            if thought:
                chat_msg = f"💬 [현재 생각] {mood}: {thought}"
    ...
    async def _execute_scouting(self):
        """
        정찰 실행 - 적절한 타이밍에 상대 기지 정찰

        정찰 타이밍:
        1. 초반 (0-30초): 빠른 정찰 (이미 _fast_scouting_20_seconds에서 처리)
        2. 중반 (30초-2분): 주기적 정찰 (30초마다)
        3. 중후반 (2분 이후): 전략적 정찰 (적 정보 부족 시)
        """
        try:
            # 적 시작 위치 확인
            if not self.enemy_start_locations or len(self.enemy_start_locations) == 0:
                # 🚀 수정: heatmap -> ScoutingSystem (통합됨)
                # Heatmap 기반 정찰은 ScoutingSystem에서 처리
                if self.scout:
                    # ScoutingSystem의 update는 on_step의 100프레임 주기에서 호출됨
                    # 여기서는 추가 정찰 로직이 필요하면 처리
                    pass
                return
            ...
            # 1. 초반 정찰 (30초-1분): 주기적 정찰
            if 30 <= game_time < 60:
                if not hasattr(self, "_last_scout_time"):
                    self._last_scout_time = 0
                if game_time - self._last_scout_time >= 30:  # 30초마다 정찰
                    should_scout = True
                    scout_reason = "주기적 정찰 (30초)"

            # 2. 중반 정찰 (1분-2분): 적 정보 부족 시 정찰
            elif 60 <= game_time < 120:
                if not hasattr(self, "_last_scout_time"):
                    self._last_scout_time = 0
                # 적 건물 정보가 부족하면 정찰
                enemy_structures_known = (
                    len(self.enemy_structures) if hasattr(self, "enemy_structures") else 0
                )
                if enemy_structures_known < 3 and game_time - self._last_scout_time >= 45:
                    should_scout = True
                    scout_reason = f"적 정보 부족 (건물 {enemy_structures_known}개만 확인)"

            # 3. 중후반 정찰 (2분 이후): 전략적 정찰
            elif game_time >= 120:
                if not hasattr(self, "_last_scout_time"):
                    self._last_scout_time = 0
                # 병력이 일정 수 이상이고, 적 정보가 부족하면 정찰
                combat_units = self.units.filter(lambda u: u.type_id in self.combat_unit_types)
                army_count = (
                    combat_units.amount
                    if hasattr(combat_units, "amount")
                    else len(list(combat_units))
                )
                if army_count >= 20 and game_time - self._last_scout_time >= 60:
                    should_scout = True
                    scout_reason = f"전략적 정찰 (병력 {army_count}기)"
            ...
            if should_scout:
                ...
                if overlords:
                    ...
                    try:
                        ...
                        if self.iteration % 50 == 0:
                            print(f"[SCOUT] [{int(game_time)}s] 대군주 정찰 시작: {scout_reason}")
                    except Exception:
                        pass
                else:
                    # 대군주가 없으면 저글링 정찰
                    ...
                    if zerglings:
                        ...
                        try:
                            ...
                            if self.iteration % 50 == 0:
                                print(
                                    f"[SCOUT] [{int(game_time)}s] 저글링 정찰 시작: {scout_reason} (대군주 없음)"
                                )
                        except Exception:
                            pass
            ...
            # 🚀 수정: heatmap -> ScoutingSystem (통합됨)
            # Heatmap 기반 정찰은 ScoutingSystem에서 처리
            # Critical scout targets are now handled by ScoutingSystem
            if self.scout:
                ...
                if target:
                    ...
                    if idle_overlords:
                        idle_overlords[0].move(target)

                # Additional overlord positioning logic for vision control
                # Position overlords on high ground for drop defense and map vision
                overlord_list = list(overlords) if hasattr(overlords, "__iter__") else []
                if len(overlord_list) > 2:
                    for i, ovie in enumerate(
                        overlord_list[2:], start=2
                    ):  # Skip first 2 (already assigned)
                        if ovie.is_idle:
                            try:
                                # Position on strategic high ground near our base
                                if self.townhalls.exists:
                                    base_pos = self.townhalls.first.position
                                    # Move to high ground position (30 units from base towards map center)
                                    high_ground_pos = base_pos.towards(
                                        self.game_info.map_center, 30
                                    )
                                    ovie.move(high_ground_pos)
                            except Exception:
                                pass
        except Exception as e:
            # 에러 발생 시 조용히 스킵 (게임 중단 방지)
    ...
    async def _morph_overseer(self):
        ...
        if overlords and self.can_afford(UnitTypeId.OVERSEER):
            ...
            if (lairs or hives) and overlords:
                if overlords:
                    overlords[0](AbilityId.MORPH_OVERSEER)
    ...
    async def _check_logic_bugs(self):
        """
        Check for logical bugs and record them with enhanced debug output

        Detects common bugs:
        1. Supply blocked without Overlord training
        2. Can afford unit but not producing
        3. Queen has energy but not injecting
        4. Resources available but no production
        """
        try:
            instance_id = getattr(self, "instance_id", 0)
            instance_tag = f"[ID:{instance_id}]"
    ...
    async def _log_game_state(self):
        """
        Log game state for replay analysis (Telemetry) - TelemetryLogger로 위임
        """
        try:
            # TelemetryLogger로 위임
    ...
    def write_log(self, message: str, level: str = "INFO", filter_key: Optional[str] = None):
        """
        Write log message to text file with filtering and size management

        Args:
            message: Log message to write
            level: Log level (INFO, WARNING, ERROR, DEBUG)
            filter_key: Filter key to check if logging is enabled (e.g., "unit_death", "attack_events")
        """
    ...
    async def _detect_unit_deaths(self):
        """
        Detect when units die and log them (if unit_death filter is enabled)
        """
    ...
    # 🔥 AGGRESSIVE ARMY PRODUCTION: Always produce units when possible
    async def _build_army_aggressive(self):
        """
        Aggressively produce combat units with reactive composition logic

        This function ensures continuous unit production by:
        1. Checking for available larvae
        2. Analyzing enemy tech and structures to determine ideal composition
        3. Producing optimal unit mix based on reactive composition
        ...
        # 2. 인구수가 막히지 않게 대군주 우선 생산 (여유 인구 5 미만일 때)
        if self.supply_left < 5 and self.supply_cap < 200:
            # type: ignore[operator] suppresses can_afford wrapper warnings
            if self.can_afford(UnitTypeId.OVERLORD) and not self.already_pending(UnitTypeId.OVERLORD):  # type: ignore[operator]
                if larvae.exists:
                    larvae_list = list(larvae)
                    if larvae_list:
        ...
        # 3. 정찰 기반 이상적 조합 결정 (캐시된 값 사용, 없으면 계산)
    ...
    async def _force_resource_dump(self):
        ...
        # 남는 돈으로 부화장(Hatchery)을 늘려 라바 공급처 확보
        # type: ignore[operator] suppresses can_afford wrapper warnings
    ...
    async def _panic_mode_production(self):
        ...
        # 가시촉수는 EarlyDefenseManager에서 처리
    ...
    async def _build_terran_counters(self):
        ...
        # 맹독충 둥지 건설 확인 (중복 건설 방지: 존재/펜딩 모두 검사)
        ...
        # 🔧 로치 워렌은 EconomyManager._build_tech_buildings()가 안전하게 처리
        # 메인 봇에서는 직접 빌드하지 않음 (중복 건설 방지 + Logic Fragmentation 해결)
    ...
    async def _build_protoss_counters(self):
        ...
        # 🔧 히드라 덴과 로치 워렌은 EconomyManager._build_tech_buildings()가 안전하게 처리
        # 메인 봇에서는 직접 빌드하지 않음 (중복 건설 방지 + Logic Fragmentation 해결)
    ...
    async def _build_zerg_counters(self):
        ...
        # 🔧 로치 워렌, 맹독충 둥지, 히드라 덴은 EconomyManager._build_tech_buildings()가 안전하게 처리
        # 메인 봇에서는 직접 빌드하지 않음 (중복 건설 방지 + Logic Fragmentation 해결)
    ...
    async def _send_game_progress_to_chat(self):
        """
        실시간 게임 진행도 채팅 알림 (5초마다 호출)

        빌드 단계, 인구수, 자원, 상대 테크 정보를 채팅창에 출력하여
        디버깅과 전략 분석을 용이하게 합니다.
        """
    ...
    async def _calculate_and_display_win_probability(self):
        """
        실시간 승리 확률 대조 및 채팅 출력 (10초마다 호출)

        상대 테크와 내 유닛 조합을 비교하여 승률을 계산하고,
        채팅창에 승률과 전략적 조언을 출력합니다.
        """
        # strategy_engine removed: win probability calculation functionality deprecated
        pass

    def _print_status(self):
        """상태 출력 - Optimized: use amount attribute"""
        instance_id = getattr(self, "instance_id", 0)
        instance_tag = f"[ID:{instance_id}]"
    ...
    def save_model_safe(self):
        ...
        try:
            # ⚠️ AI Arena 환경: 상대 경로로 models 디렉토리 지정
    ...
    def _collect_state(self) -> np.ndarray:
        ...
        CPU/GPU Load Balancing: CPU가 데이터를 빠르게 준비하여 GPU가 연산에 집중할 수 있도록 최적화

        Input State Vector (5-dimensional to match saved model):
            - Minerals (미네랄)
            - Vespene Gas (가스)
            - Supply Used (공급량 사용)
            - Drone Count (드론 수)
        ...
        Note: Tech Level removed to match saved model structure (5 inputs)

        Returns:
            np.ndarray: 상태 배열 [minerals, vespene, supply, drones, army]
        """
        try:
            ...
            # 1. 자원 (Minerals, Vespene) - Direct access (fast)
            minerals = float(self.minerals)
            vespene = float(self.vespene)

            # 2. 공급량 (Supply Used) - Direct access (fast)
            supply_used = float(self.supply_used)

            # 3. 드론 수 (Drone Count) - Cached for performance
            if hasattr(self, "_cached_worker_count"):
            ...
            # 4. 병력 수 (Army Count) - Cached for performance
            # 일꾼, 대군주, 여왕 제외
            if hasattr(self, "_cached_army_count"):
                # Use cached value if available (updated periodically)
            ...
            # 상태 벡터 생성 (5-dimensional: matches saved model structure)
            # Tech Level removed to match saved model (5 inputs instead of 6)
            state = np.array(
    ...
    def choose_action(self, state=None):
        ...
        if self.train_mode and random.random() < self.epsilon:
            ...
            # 신경망이 추천하는 최적의 행동 (Exploitation)
            # GPU 활용 최적화: 추론 빈도 제한 (24프레임마다 = ~1.1초마다, GPU 사용률 30%)
    ...
    async def _autonomous_personality_chat(self):
        ...
        try:
            ...
            # 4. 감정에 따른 시각적 효과를 채팅으로 표현
            # 시각적 원 대신 색상 이모지로 감정 표현
            try:
                ...
                # 시각적 효과 실패는 게임을 중단시키지 않음
                pass

        except Exception as e:
            # 에러 발생 시 초기화 실패로 이어지지 않게 로그만 출력
    ...
    async def on_end(self, game_result):
        ...
        def _retry(func, description: str, retries: int = 3, delay: float = 1.0):
            """Retry a sync operation (e.g., file write) with small backoff."""
            for attempt in range(retries):
                try:
                    func()
                    return True
        ...
        try:
            if hasattr(self, "strategy_analyzer") and self.strategy_analyzer:
                ...
                # 🚀 수정: strategy_analyzer가 None일 수 있으므로 안전하게 체크
                if self.strategy_analyzer is not None:
                    try:
        ...
        # 📊 최종 통계 저장 (main.py에서 사용) - Optimized: use amount attribute
        try:
            ...
            self.final_stats = {
                ...
                "game_time": int(self.time),
                "loss_reason": loss_reason,  # 패배 원인 추가
            }
        except Exception as e:
        ...
        if self.use_neural_network and self.neural_network is not None:
            try:
                ...
                if (
                    ...
                    # Lurker를 생산했으면 보너스
                    lurker_bonus = min(0.5, lurker_count * 0.1)  # 최대 +0.5 보너스
                    base_reward += lurker_bonus
                    print(
                        f"[TRAIN] ✅ Lurker 생산: +{lurker_bonus:.2f} 보너스 ({lurker_count}마리)"
                    )

                # 드론 수 보너스 - Optimized: use amount attribute
                ...
                # 최종 보상 계산 (패배 원인 패널티 포함)
                final_reward = (
                    base_reward + loss_penalty + lurker_penalty + drone_bonus + build_order_reward
                )

                # 보상 출력 (패배 원인 및 보상 포함)
                if str(game_result) == "Victory":
                    print(
                        f"[TRAIN] 🎊 승리! Base: +{base_reward:.1f}, Drone: +{drone_bonus:.2f}, Build Order: {build_order_reward:+.2f}, Total: +{final_reward:.2f}"
                    )
                elif str(game_result) == "Defeat":
                    print(
                        f"[TRAIN] 💀 패배... Base: {base_reward:.1f}, Loss Reason: {loss_penalty:.1f}, Lurker: {lurker_penalty:.1f}, Drone: +{drone_bonus:.2f}, Build Order: {build_order_reward:+.2f}, Total: {final_reward:.2f}"
                    )

                # 모델 업데이트 (REINFORCE)
                self.neural_network.finish_episode(final_reward)

                # 모델 저장: 각 인스턴스별로 별도 파일에 저장 (병렬 실행 충돌 방지)
                # zerg_net_0.pt, zerg_net_1.pt, zerg_net_2.pt 등으로 분리 저장
        ...
        # 📊 패배 원인 통계 기록 (training_stats.json) - TelemetryLogger로 위임
        try:
            # TelemetryLogger가 모든 통계 기록 처리
        ...
        # NOTE: Removed Victory Screen Pause to prevent ladder timeouts.
    ...
    def _calculate_build_order_reward(self) -> float:
        ...
        try:
            ...
            # 4. 28 서플라이: 세 번째 해처리 (3rd Hatchery)
            target_supply = 28
            ...
            if actual_supply is not None:
                ...
                reward = -0.2  # 완화: -0.3 -> -0.2
                total_reward += reward
                print(f"[BUILD REWARD] ❌ 세 번째 해처리 미실행: {reward:.2f}")

            # 5. 30 서플라이: 발업 (Metabolic Boost)
            target_supply = 30
            ...
            if actual_supply is not None:
                supply_diff = actual_supply - target_supply
                if abs(supply_diff) <= 4:  # ±4까지 허용 (이전: ±2)
                    reward = 0.2
                    print(
    ...
    async def _check_for_surrender(self) -> bool:
        ...
        try:
            ...
            # Condition 2: 생산 시설 전멸 (Production Dead)
            # 부화장이 하나도 없으면 항복
            townhalls = self.townhalls.amount if hasattr(self, "townhalls") else 0
            if townhalls == 0:
                try:
            ...
            # Condition 3: 병력 차이 절망 + 적 본진 근접 (Army Overwhelmed)
            # 게임 시간 5분(300초) 이상, 내 병력이 없고 적이 본진 근처에 있을 때
            game_time = self.time if hasattr(self, "time") else 0
            if game_time > 300:  # 5 minutes
                # 전투 유닛 확인 (화이트리스트 방식 - 저글링 이상 전투 유닛만)
                army_units = []
                try:
                    if hasattr(self, "units"):
                        for unit in self.units:
            ...
            # Condition 4: 초반 3분 내 부화장 전멸 (Rage Quit)
            # 게임 시작 3분(180초) 이내에 부화장이 모두 파괴되면 항복
            game_time = self.time if hasattr(self, "time") else 0
            ...
            # Condition 5: 장기전 자원 고갈 (Resource Exhausted)
            # 20분(1200초) 이상 경과, 자원 10원 미만, 라바 없음
            if game_time > 1200:
                larvae = self.units(UnitTypeId.LARVA)
